<template>
	<view :class="isDark?'darkBg':''">
		<cu-custom :bgColor="isDark?'bg-black':'bg-red'" isBack="true">
			...
			<block slot="content">我分享的成绩</block>
		</cu-custom>
		...
		<view :class="isDark?'dark':''">
			<view>
				<view class="cu-bar  solid-bottom" style="font-weight:bold" :class="isDark?'dark':'bg-white'">
					<view class="action">
						<text class="cuIcon-titles text-red"></text> 我分享的成绩
				...
				<view class="margin flex text-center text-grey  shadow-warp cu-list menu card-menu"
					...
					<view class=" padding flex flex-direction" :class="isDark?'dark':'bg-white'">
						<button class="cu-btn round bg-red text-white lg">总数:{{totalList.length}}</button>
					</view>
					...
					<view style="display: flex;">
						...
						<view>
							...
							<button class="cu-btn bg-red  " style="margin-left:65rpx;margin-top:20rpx"
								v-if="val.deleteTime==null" :data-key="val.shareKey" @tap="shareCancel">取消分享</button>
						</view>
					</view>
				</view>
	...
	export default {
		...
		methods: {
			...
			shareCancel: function(e) {
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							_this.$schoolApi.scoreShareCancel(form).then(response => {
			...
			refreshInfo: function() {
				let _this = this
				_this.$schoolApi.scoreShareInfo().then(response => {
