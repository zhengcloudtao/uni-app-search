	...
	<view :class="isDark?'darkBg':''">
		...
		<view :class="isDark?'dark':''">
			...
			<view class="cu-bar bg-white solid-bottom" style="font-weight:bold" :class="isDark?'dark':'bg-white'">
				<view class="action">
					<text class="cuIcon-titles text-brown"></text>课表总揽
	...
	export default {
		...
		onShow() {
			...
			if (myCustomTimetable.length != undefined) {
				_this.customNum = myCustomTimetable.length;
				_this.customList = myCustomTimetable
			}
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							...
							_this.$store.commit('myCustomTimetable', arr)
							uni.showToast({
								title: "删除成功",
								icon: "none"
							})
				...
				uni.showModal({
					...
					success: function(res) {
						if (res.confirm) {
							...
							_this.list = newArr
							_this.$store.commit('myTimetable', newArr)
							uni.showToast({
								title: "删除成功",
								icon: false
							})
